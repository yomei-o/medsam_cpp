...
// ---- SHARED forward: image embedding [1,256,64,64] + prompt -> masks[4,256,256], iou[4] ----
// points: coords (x,y) in the 1024-input space, labels (1=fg,0=bg); a pad point is appended.
// box (optional, MedSAM): [x0,y0,x1,y1] in 1024 space -> two corner tokens (+point_embeddings[2],[3]),
//   no pad point. mask[0] is the single-mask output (multimask_output=False), mask[1..3] the multimask.
struct SamOut { Tensor masks, iou, sparse, image_pe; };
inline SamOut sam_decode(const Tensor& img_embed, const std::vector<float>& pts, const std::vector<int>& labels, SamW& w, const float* box = nullptr) {
  ...
  if (box) {                                                         // box: two corners, no pad point
    std::vector<float> bc = {(box[0] + 0.5f) / c.input, (box[1] + 0.5f) / c.input, (box[2] + 0.5f) / c.input, (box[3] + 0.5f) / c.input};
    Tensor bpe = pe_encode(bc, 2, G);
    srows.push_back(add(slice_rows(bpe, 0, 1), ptemb[2]));           // top-left corner
    srows.push_back(add(slice_rows(bpe, 1, 2), ptemb[3]));           // bottom-right corner
  } else {                                                           // points: pe + point_embeddings[label], then a pad point
